...
def get_gmt8_timestamp():
    ...
    try:
        ...
        return gmt8_now.strftime("%Y-%m-%d %H:%M:%S")
    except Exception:
        # 如果 pytz 不可用，回退到本地时间（带时区警告）
        print("[WARN] pytz 不可用，使用本地时间，可能存在时区不一致")
        return datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")


def log(level, module, message):
    """统一日志记录，强制使用 GMT+8 时区"""
    timestamp = get_gmt8_timestamp()
    log_entry = f"[{timestamp} GMT+8] [{module}:{level}] {message}\n"
